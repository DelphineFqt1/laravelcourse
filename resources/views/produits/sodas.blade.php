    ...
    @if ($sodas->count() > 0)
        ...
        <div class="lg:w-1/4 md:w-1/2 p-4 w-full">
            <a class="block relative h-48 rounded overflow-hidden">
              <img alt="ecommerce" class="object-cover object-center block" src= {{ $soda->image}}>
            </a>
            <div class="mt-4">
              <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1">{{ $soda->name }}</h3>
              <h2 class="text-gray-900 title-font text-lg font-medium">{{ $soda->description }}</h2>
              <p class="mt-1">{{ $soda  ->price }}€</p>
            </div>
        </div>
        @endforeach
        @else
      <svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" fill="currentColor" class="bi bi-emoji-frown centered" viewBox="0 0 16 16">
        ...
        <span>Aucun soda pour le moment...</span>
      @endif
        </div>
    </div>
</section>
